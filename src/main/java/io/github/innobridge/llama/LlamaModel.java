 ...
 * Thus, this class is an {@link AutoCloseable}, in order to de-allocate the
 * memory when it is no longer being needed.
 * <p>
 * The main functionality of this class is:
 * <ul>
 * <li>Streaming answers (and probabilities) via
 * {@link #generate(InferenceParameters)}</li>
 * <li>Creating whole responses to prompts via
 * {@link #complete(InferenceParameters)}</li>
 * <li>Creating embeddings via {@link #embed(String)} (make sure to configure
 * {@link ModelParameters#setEmbedding(boolean)}</li>
 * <li>Accessing the tokenizer via {@link #encode(String)} and
 * {@link #decode(int[])}</li>
 * </ul>
 */
public class LlamaModel implements AutoCloseable {
   ...
   * <li>{@link ModelParameters#setHuggingFaceRepository(String)}},
   * {@link ModelParameters#setHuggingFaceFile(String)}</li>
   * </ul>
   *
  ...
  /**
   * Generate and return a whole answer with custom parameters. Note, that the
   * prompt isn't preprocessed in any
  ...
  /**
   * Generate and stream outputs with custom inference parameters. Note, that the
   * prompt isn't preprocessed in any
   * way, nothing like "User: ", "###Instruction", etc. is added.
   *
  ...
  /**
   * Get the embedding of a string. Note, that the prompt isn't preprocessed in
   * any way, nothing like
   ...
   * Per default, log messages are written in JSON to stdout. Note, that in text
   * mode the callback will be also
   * invoked with log messages of the GGML backend, while JSON mode can only
   * access request log messages.
   * In JSON mode, GGML messages will still be written to stdout.
   * To only change the log format but keep logging to stdout, the given callback
   * can be <code>null</code>.
   * To disable logging, pass an empty callback, i.e.,
   * <code>(level, msg) -> {}</code>.
   *
