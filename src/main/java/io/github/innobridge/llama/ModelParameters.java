...
public final class ModelParameters extends JsonParameters {
  ...
  /**
   * Set the number of threads to use during draft generation (default: same as
   * {@link #setNThreads(int)})
   */
  ...
  /**
   * Set the number of threads to use during batch and prompt processing (default:
   * same as {@link #setNThreads(int)})
   */
  ...
  /**
   * Set the number of tokens to predict (default: -1, -1 = infinity, -2 = until
   * context filled)
   */
  ...
  /**
   * Set the number of tokens to keep from the initial prompt (default: 0, -1 =
   * all)
   */
  ...
  /**
   * Set the number of layers to store in VRAM for the draft model (-1 - use
   * default)
   */
  ...
  /**
   * Set the RoPE base frequency, used by NTK-aware scaling (default: loaded from
   * model)
   */
  ...
  /**
   * Set the YaRN extrapolation mix factor (default: 1.0, 0.0 = full
   * interpolation)
   */
  ...
  /**
   * Set the YaRN original context size of model (default: 0 = model training
   * context size)
   */
   ...
   * <li><b>isolate</b>: only spawn threads on CPUs on the node that execution
   * started on</li>
   * <li><b>numactl</b>: use the CPU map provided by numactl</li>
   * </ul>
   * If run without this previously, it is recommended to drop the system page
  ...
  /**
   * Set the RoPE frequency scaling method, defaults to linear unless specified by
   * the model
   */
   ...
   * Note, that this requires the library to be built with CURL
   * (<code>-DLLAMA_CURL=ON</code>).
   */
  ...
  /**
   * Set a Hugging Face model file to use (default: unused, see
   * {@link #setHuggingFaceRepository(String)})
   */
  ...
  /**
   * Set path to static lookup cache to use for lookup decoding (not updated by
   * generation)
   */
  ...
  /**
   * Set path to dynamic lookup cache to use for lookup decoding (updated by
   * generation)
   */
  ...
  /**
   * Whether to enable continuous batching (also called "dynamic batching")
   * (default: disabled)
   */
  ...
  /**
   * Whether to ignore end of stream token and continue generating (implies
   * --logit-bias 2-inf)
   */
  ...
  /**
   * Whether to use memory-map model (faster load but may increase pageouts if not
   * using mlock)
   */
  ...
  /**
   * Whether to force the system to keep model in RAM rather than swapping or
   * compressing
   */
