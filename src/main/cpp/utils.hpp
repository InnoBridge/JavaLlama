...
#define LOG_ERROR(MSG, ...)                                                    \
  server_log(GGML_LOG_LEVEL_ERROR, __func__, __LINE__, MSG, __VA_ARGS__)
#define LOG_WARNING(MSG, ...)                                                  \
  server_log(GGML_LOG_LEVEL_WARN, __func__, __LINE__, MSG, __VA_ARGS__)
#define LOG_INFO(MSG, ...)                                                     \
...
static inline void server_log(ggml_log_level level, const char *function,
  ...
  if (log_json) {
    json log = json{
        {"msg", message},
#if SERVER_VERBOSE
        {"ts", time(nullptr)}, {"level", log_level_to_string(level)},
        {"tid", ss_tid.str()}, {"function", function},
        {"line", line},
#endif
    };
    ...
    if (!extra.empty()) {
      for (const auto &el : extra.items()) {
        const std::string value =
            el.value().dump(-1, ' ', false, json::error_handler_t::replace);
        ss << " " << el.key() << "=" << value;
      }
    }

#if SERVER_VERBOSE
    ss << " | ts " << time(nullptr) << " | tid " << ss_tid.str() << " | "
       << function << " line " << line;
#endif
...
static json oaicompat_completion_params_parse(
  ...
  llama_params["prompt"] =
      format_chat(model, chat_template, body.at("messages"));
  ...
  if (body.contains("response_format")) {
    ...
    std::string response_type =
        json_value(response_format, "type", std::string());
    if (response_type == "json_object") {
      llama_params["json_schema"] =
          json_value(response_format, "schema", json::object());
    } else if (!response_type.empty() && response_type != "text") {
      throw std::runtime_error("response_format type must be one of \"text\" "
                               "or \"json_object\", but got: " +
                               response_type);
    }
  }
  ...
  // TODO: The response format of this option is not yet OAI-compatible, but
  // seems like no one really using it; We may need to fix it in the future
  ...
  static const std::vector<std::string> unsupported_params{"tools",
                                                           "tool_choice"};
  ...
  // "tfs_z",... via OAI endpoint. See "launch_slot_with_task()" for a complete
  // list of params supported by llama.cpp
...
format_partial_response_oaicompat(json result,
  ...
  if (!finish_reason.empty()) {
    ...
    ret.push_back({"usage", json{{"completion_tokens", num_tokens_predicted},
                                 {"prompt_tokens", num_prompt_tokens},
                                 {"total_tokens",
                                  num_tokens_predicted + num_prompt_tokens}}});
  }
...
static json format_embeddings_response_oaicompat(const json &request,
  ...
  for (auto &elem : embeddings) {
    data.push_back(
        json{{"embedding", json_value(elem, "embedding", json::array())},
             {"index", i++},
             {"object", "embedding"}});
  }

  json res = json{{"model", json_value(request, "model",
                                       std::string(DEFAULT_OAICOMPAT_MODEL))},
                  {"object", "list"},
