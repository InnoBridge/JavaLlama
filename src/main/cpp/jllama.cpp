...
// We store some references to Java classes and their fields/methods here to
// speed up things for later and to fail early on if anything can't be found.
// This happens when the JVM loads the shared library (see `JNI_OnLoad`). The
// references remain valid throughout the whole life of the shared library, on
// `JNI_OnUnload` they are released.
 ...
 * `env->NewString` or `env-NewString`, but we directly send the bytes and do
 * the conversion in Java. Unfortunately, there isn't a nice/standardized way to
...
JNIEnv *get_jni_env() {
  JNIEnv *env = nullptr;
  if (g_vm == nullptr || g_vm->GetEnv(reinterpret_cast<void **>(&env),
 ...
 */
void log_callback_trampoline(ggml_log_level level, const char *text,
                             void *user_data) {
 ...
 * In order to use any of the new JNI functions, a native library must export a
 `JNI_OnLoad` function that returns
 * `JNI_VERSION_1_2`. If the native library does not export a JNI_OnLoad
 function, the VM assumes that the library
 * only requires JNI version `JNI_VERSION_1_1`. If the VM does not recognize the
 version number returned by `JNI_OnLoad`, the VM will unload the library and act
 as if the library was never loaded.
 */
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  if (!(c_llama_model && c_llama_iterator && c_standard_charsets && c_output &&
        c_string && c_hash_map && c_map && c_set && c_entry && c_iterator &&
        c_integer && c_float && c_biconsumer && c_llama_error && c_log_level &&
  ...
  o_log_format_json =
      env->GetStaticObjectField(c_log_format, f_log_format_json);
  o_log_format_text =
      env->GetStaticObjectField(c_log_format, f_log_format_text);
 ...
 * services, and refrain from arbitrary Java call-backs. Note that `JNI_OnLoad`
 * and `JNI_OnUnload` are two functions optionally supplied by JNI libraries,
...
JNIEXPORT void JNICALL Java_io_github_innobridge_llama_client_LlamaModel_loadModel(
  ...
  if (params.chat_template.empty()) {
    if (!ctx_server->validate_model_chat_template()) {
      LOG_ERROR("The chat template that comes with this model is not yet "
                "supported, falling back to chatml. This "
  ...
  // print sample chat example to make it clear which template is used
  {
    LOG_INFO("chat template",
             {
                 {"chat_example", llama_chat_format_example(
                                      ctx_server->model, params.chat_template)},
...
Java_io_github_innobridge_llama_client_LlamaModel_requestCompletion(JNIEnv *env,
  ...
  std::string c_params = parse_jstring(env, jparams);
  json json_params = json::parse(c_params);
  const bool infill = json_params.contains("input_prefix") ||
                      json_params.contains("input_suffix");

  if (json_params.value("use_chat_template", false)) {
    json chat;
    chat.push_back(
        {{"role", "system"}, {"content", ctx_server->system_prompt}});
    chat.push_back({{"role", "user"}, {"content", json_params["prompt"]}});
    json_params["prompt"] =
        format_chat(ctx_server->model, ctx_server->params.chat_template, chat);
  }
...
Java_io_github_innobridge_llama_client_LlamaModel_receiveCompletion(JNIEnv *env,
  ...
  return env->NewObject(c_output, cc_output, jbytes, o_probabilities,
                        result.stop);
}
...
JNIEXPORT jfloatArray JNICALL Java_io_github_innobridge_llama_client_LlamaModel_embed(
  ...
  if (!ctx_server->params.embedding) {
    env->ThrowNew(c_llama_error, "model was not loaded with embedding support "
                                 "(see ModelParameters#setEmbedding(boolean))");
    return nullptr;
  }
  ...
  std::vector<float> embedding =
      result.data["embedding"].get<std::vector<float>>();
  ...
  env->SetFloatArrayRegion(j_embedding, 0, embedding_size,
...
JNIEXPORT jintArray JNICALL Java_io_github_innobridge_llama_client_LlamaModel_encode(
  ...
  env->SetIntArrayRegion(java_tokens, 0, token_size,
...
Java_io_github_innobridge_llama_client_LlamaModel_decodeBytes(JNIEnv *env, jobject obj,
  ...
  std::string text =
      tokens_to_str(ctx_server->ctx, tokens.cbegin(), tokens.cend());
...
JNIEXPORT void JNICALL Java_io_github_innobridge_llama_client_LlamaModel_setLogger(
  ...
  if (jcallback == nullptr) {
    ...
    log_callback = [](enum ggml_log_level level, const char *text,
                      void *user_data) {
      JNIEnv *env = get_jni_env();
      jstring message = env->NewStringUTF(text);
      jobject log_level = log_level_to_jobject(level);
      env->CallVoidMethod(o_log_callback, m_biconsumer_accept, log_level,
                          message);
