...
struct server_context {
  ...
  server_slot *get_available_slot(const std::string &prompt) {
    ...
    if (ret == nullptr && slot_prompt_similarity != 0.0f && !prompt.empty()) {
      ...
      for (server_slot &slot : slots) {
        ...
        // length of the Longest Common Prefix between the current slot's prompt
        // and the input prompt
        int lcp_len = common_part(slot_prompt, prompt);

        // fraction of the common substring length compared to the current
        // slot's prompt length
  ...
  bool launch_slot_with_task(server_slot &slot, const server_task &task) {
    ...
    // (but individual requests can still override them)
    ...
    slot.params.n_predict =
        json_value(data, "n_predict",
                   json_value(data, "max_tokens", default_params.n_predict));
    ...
    if (!task.infill) {
      ...
      if (prompt == data.end()) {
        send_error(task, "\"prompt\" must be provided",
                   ERROR_TYPE_INVALID_REQUEST);
        return false;
      }

      if ((prompt->is_string()) ||
          (prompt->is_array() && prompt->size() == 1 &&
           prompt->at(0).is_string()) ||
          (prompt->is_array() && !prompt->empty() &&
           prompt->at(0).is_number_integer())) {
        slot.prompt = *prompt;
      } else {
  ...
  bool process_token(completion_token_output &result, server_slot &slot) {
    ...
    const std::string token_str =
        llama_token_to_piece(ctx, result.tok, params.special);
  ...
  void process_single_task(const server_task &task) {
    ...
    case SERVER_TASK_TYPE_COMPLETION: {
      ...
      if (id_slot != -1) {
        ...
        std::string prompt;
        if (task.data.contains("prompt") &&
            task.data.at("prompt").is_string()) {
  ...
  void update_slots() {
        ...
        if (slot.state == SLOT_STATE_IDLE &&
          ...
          if (prompt_tokens.empty()) {
            ...
            if (slot.infill) {
              ...
              suffix_tokens.insert(suffix_tokens.begin(),
    ...
    for (int32_t i = 0; i < batch.n_tokens; i += n_batch) {
      ...
      if (ret != 0) {
        ...
        LOG_WARNING(
            "failed to find free space in the KV cache, retrying with smaller "
            "batch size - try "
...
static void server_params_parse(json jparams, gpt_params &params) {
  ...
  params.n_threads_draft =
      json_value(jparams, "n_threads_draft", default_params.n_threads_draft);
  params.n_threads_batch =
      json_value(jparams, "n_threads_batch", default_params.n_threads_batch);
  params.n_threads_batch_draft = json_value(
      jparams, "n_threads_batch_draft", default_params.n_threads_batch_draft);
  ...
  params.n_parallel =
      json_value(jparams, "n_parallel", default_params.n_parallel);
  params.n_sequences =
      json_value(jparams, "n_sequences", default_params.n_sequences);
  params.p_split = json_value(jparams, "p_split", default_params.p_split);
  params.grp_attn_n =
      json_value(jparams, "grp_attn_n", default_params.grp_attn_n);
  params.grp_attn_w =
      json_value(jparams, "grp_attn_w", default_params.grp_attn_w);
  params.n_print = json_value(jparams, "n_print", default_params.n_print);
  params.rope_freq_base =
      json_value(jparams, "rope_freq_base", default_params.rope_freq_base);
  params.rope_freq_scale =
      json_value(jparams, "rope_freq_scale", default_params.rope_freq_scale);
  params.yarn_ext_factor =
      json_value(jparams, "yarn_ext_factor", default_params.yarn_ext_factor);
  params.yarn_attn_factor =
      json_value(jparams, "yarn_attn_factor", default_params.yarn_attn_factor);
  params.yarn_beta_fast =
      json_value(jparams, "yarn_beta_fast", default_params.yarn_beta_fast);
  params.yarn_beta_slow =
      json_value(jparams, "yarn_beta_slow", default_params.yarn_beta_slow);
  params.yarn_orig_ctx =
      json_value(jparams, "yarn_orig_ctx", default_params.yarn_orig_ctx);
  params.defrag_thold =
      json_value(jparams, "defrag_thold", default_params.defrag_thold);
  params.numa = json_value(jparams, "numa", default_params.numa);
  params.rope_scaling_type = json_value(jparams, "rope_scaling_type",
                                        default_params.rope_scaling_type);
  params.pooling_type =
      json_value(jparams, "pooling_type", default_params.pooling_type);
  params.model = json_value(jparams, "model", default_params.model);
  params.model_draft =
      json_value(jparams, "model_draft", default_params.model_draft);
  params.model_alias =
      json_value(jparams, "model_alias", default_params.model_alias);
  ...
  params.prompt_file =
      json_value(jparams, "prompt_file", default_params.prompt_file);
  params.path_prompt_cache = json_value(jparams, "path_prompt_cache",
                                        default_params.path_prompt_cache);
  params.input_prefix =
      json_value(jparams, "input_prefix", default_params.input_prefix);
  params.input_suffix =
      json_value(jparams, "input_suffix", default_params.input_suffix);
  params.antiprompt =
      json_value(jparams, "antiprompt", default_params.antiprompt);
  params.lookup_cache_static = json_value(jparams, "lookup_cache_static",
                                          default_params.lookup_cache_static);
  params.lookup_cache_dynamic = json_value(jparams, "lookup_cache_dynamic",
                                           default_params.lookup_cache_dynamic);
  params.logits_file =
      json_value(jparams, "logits_file", default_params.logits_file);
  params.lora_adapter =
      json_value(jparams, "lora_adapter", default_params.lora_adapter);
  ...
  params.cont_batching =
      json_value(jparams, "cont_batching", default_params.cont_batching);
  params.flash_attn =
      json_value(jparams, "flash_attn", default_params.flash_attn);
  params.input_prefix_bos =
      json_value(jparams, "input_prefix_bos", default_params.input_prefix_bos);
  params.ignore_eos =
      json_value(jparams, "ignore_eos", default_params.ignore_eos);
  ...
  params.no_kv_offload =
      json_value(jparams, "no_kv_offload", default_params.no_kv_offload);
  params.system_prompt =
      json_value(jparams, "system_prompt", default_params.system_prompt);
  ...
  if (jparams.contains("split_mode")) {
    params.split_mode =
        json_value(jparams, "split_mode", default_params.split_mode);
// todo: the definition checks here currently don't work due to cmake visibility
// reasons
#ifndef GGML_USE_CUDA
    fprintf(stderr, "warning: llama.cpp was compiled without CUDA. Setting the "
  ...
  if (jparams.contains("tensor_split")) {
    ...
    std::vector<float> tensor_split =
        jparams["tensor_split"].get<std::vector<float>>();
...
#else
    LOG_WARNING("llama.cpp was compiled without CUDA. It is not possible to "
                "set a tensor split.\n",
                {});
#endif // GGML_USE_CUDA
  }
